...
const VendorPricingOverrides = ({ onUpdate }) => {
  ...
  const fetchVendors = async (page = 1, search = vendorSearch, silent = false) => {
    if (!silent) setLoadingVendors(true);
    try {
      ...
      if (!silent) setLoadingVendors(false);
    }
  };
  ...
  const handleSaveVendorPricing = async (e) => {
    ...
    try {
      ...
      fetchVendors(vendorPagination.page, vendorSearch, true);
      if (onUpdate) onUpdate();
    } catch (err) {
      showToast(err.message || 'Failed to update vendor pricing', 'error');
    }
  };
