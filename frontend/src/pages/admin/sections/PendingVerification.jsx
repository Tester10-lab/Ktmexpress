...
const PendingVerification = () => {
  ...
  const openEditModal = (pkg) => {
    const defaultVal = pkg.verificationDraft || pkg.riderSubmission || pkg;
    setCurrentPkg(pkg);
    setForm({
  ...
  const handleReopen = async (id) => {
    if (!window.confirm('Are you sure you want to reopen verification for this package? It will unlock the record for editing.')) return;
    try {
      await api.post(`/admin/packages/${id}/reopen`);
      showToast('Verification reopened successfully.', 'success');
      fetchPackages(true);
      fetchKPIs();
    } catch (e) {
      showToast(e.response?.data?.message || 'Failed to reopen package', 'error');
    }
  };
                            ...
                            {activeTab === 'pending' ? (
                              ...
                              <>
                                ...
                                {p.deliveryVerificationStatus === 'Verified' && (
                                  <button
                                    onClick={() => handleReopen(p._id)}
                                    className="btn-sm py-1 px-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded text-[10px] font-bold flex items-center gap-1"
                                ...
                                {p.verificationAudit.map((audit, idx) => (
                                  <div key={audit._id || idx} className="p-3 bg-white rounded-lg border border-slate-200 shadow-sm text-xs grid grid-cols-1 md:grid-cols-4 gap-4">
                                    <div>
                                      ...
                                      <div className="font-bold text-slate-800 mt-1">{audit.action === 'Reopen' ? '🔄 Reopened' : '✅ Verified'}</div>
