...
const AdminScanHistory = () => {
  ...
  const fetchHistory = async (currentFilter = filter) => {
    setLoading(true);
    try {
      const q = new URLSearchParams();
      if (currentFilter.role) q.append('role', currentFilter.role);
      if (currentFilter.trackingCode) q.append('trackingCode', currentFilter.trackingCode.trim());
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative w-full sm:w-auto">
            ...
            <input 
              ...
              onKeyDown={e => e.key === 'Enter' && fetchHistory({ ...filter, trackingCode: e.target.value })} 
            />
          </div>
              ...
              : history.length === 0 ? <tr><td colSpan="5" className="px-6 py-12 text-center text-slate-500">No scan records found.</td></tr>
                : history.map(ev => (
                  <tr key={ev._id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4"><TrackingLink code={ev.trackingCode} /></td>
