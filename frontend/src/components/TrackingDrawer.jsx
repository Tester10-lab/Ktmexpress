          ...
          {shopData ? (
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-3">
                {shopData.packages && shopData.packages.map((item, idx) => {
                  const pData = (item.packageId && typeof item.packageId === 'object') ? item.packageId : item;
                  const tCode = pData.trackingCode || item.trackingCode || `PKG-${idx+1}`;
                  const cName = pData.customerName || item.customerName || 'Customer';
                  const cAddress = pData.address || item.address || '';
                  const cCity = pData.city || item.city || '';
                  const cPhone = pData.customerPhone || item.customerPhone || '';
                  const pAmount = pData.amount !== undefined ? pData.amount : item.amount;
                  const pStatus = pData.status || item.status || 'Pending';

                  return (
                    <div key={idx} className="bg-slate-50 border border-slate-200 rounded-xl p-4 shadow-xs hover:border-brand-300 transition-colors">
                      <div className="flex justify-between items-start mb-2 gap-2">
                        <button 
                          onClick={() => openTracking(tCode)}
                          className="text-sm font-black font-mono text-brand-600 hover:text-brand-700 hover:underline flex items-center gap-1.5"
                        >
                      ...
                      <div className="text-xs text-slate-700 space-y-1">
                        <p className="font-bold text-slate-900 text-sm">{cName}</p>
                        {(cAddress || cCity) && (
                          <p className="text-slate-600 flex items-start gap-1">
                            <MapPin className="w-3 h-3 text-slate-400 mt-0.5 shrink-0" />
                            <span>{cAddress}{cCity ? `, ${cCity}` : ''}</span>
                          </p>
                        )}
                        {cPhone && (
                        ...
                        {pAmount !== undefined && (
                          <div className="pt-1.5 mt-1.5 border-t border-slate-200/60 flex justify-between items-center text-xs">
                            <span className="text-slate-500 font-medium">To Collect (COD):</span>
                            <span className="font-bold text-emerald-700 font-mono text-sm">Rs. {Number(pAmount).toLocaleString()}</span>
                          </div>
                        )}
                      </div>
                    </div>
